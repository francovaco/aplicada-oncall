...
import cierreDeAccionResolve from './cierre-de-accion-routing-resolve.service';

describe('CierreDeAccion routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICierreDeAccion returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          cierreDeAccionResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(cierreDeAccionResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
