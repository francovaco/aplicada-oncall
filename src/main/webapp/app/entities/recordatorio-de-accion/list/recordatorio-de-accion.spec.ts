...
import { sampleWithRequiredData } from '../recordatorio-de-accion.test-samples';
import { RecordatorioDeAccionService } from '../service/recordatorio-de-accion.service';

import { RecordatorioDeAccion } from './recordatorio-de-accion';

vitest.useFakeTimers();

describe('RecordatorioDeAccion Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: RecordatorioDeAccion;
  let fixture: ComponentFixture<RecordatorioDeAccion>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(RecordatorioDeAccion);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 30389 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.recordatorioDeAccions()[0]).toEqual(expect.objectContaining({ id: 30389 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 30389 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.recordatorioDeAccions()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 30389 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to recordatorioDeAccionService', () => {
      const entity = { id: 30389 };
      vitest.spyOn(service, 'getRecordatorioDeAccionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getRecordatorioDeAccionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.recordatorioDeAccionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
