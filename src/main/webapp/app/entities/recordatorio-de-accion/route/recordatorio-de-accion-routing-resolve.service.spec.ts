...
import recordatorioDeAccionResolve from './recordatorio-de-accion-routing-resolve.service';

describe('RecordatorioDeAccion routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IRecordatorioDeAccion returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          recordatorioDeAccionResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(recordatorioDeAccionResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
