...
import asignacionDeAccionResolve from './asignacion-de-accion-routing-resolve.service';

describe('AsignacionDeAccion routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IAsignacionDeAccion returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          asignacionDeAccionResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(asignacionDeAccionResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
