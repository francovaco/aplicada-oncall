...
import { sampleWithRequiredData } from '../asignacion-de-accion.test-samples';
import { AsignacionDeAccionService } from '../service/asignacion-de-accion.service';

import { AsignacionDeAccion } from './asignacion-de-accion';

vitest.useFakeTimers();

describe('AsignacionDeAccion Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AsignacionDeAccion;
  let fixture: ComponentFixture<AsignacionDeAccion>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AsignacionDeAccion);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 11301 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.asignacionDeAccions()[0]).toEqual(expect.objectContaining({ id: 11301 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 11301 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.asignacionDeAccions()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 11301 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to asignacionDeAccionService', () => {
      ...
      vitest.spyOn(service, 'getAsignacionDeAccionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAsignacionDeAccionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.asignacionDeAccionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.asignacionDeAccionsParams()).toMatchObject(
